...
### Grid Sync
The bridge's `Grid Size` property is bidirectionally bound to `Gizmo.Settings.GridSpacing` in the s&box editor and `View3D.overlay.grid_scale` in Blender (display scale = `grid_size / scale_factor`). Change it in either editor and both update.

In Blender: panel slider, `1 2 4 8 16 32 64 128` preset buttons, or `[` / `]` to halve / double.
In s&box: drag the GridSpacing slider in the gizmo settings — Blender follows.

s&box's `Gizmo.Settings.GridSpacing` range is `0.125–128`. Bridge values above 128 are clamped on the s&box side; the Blender property retains the original value.
