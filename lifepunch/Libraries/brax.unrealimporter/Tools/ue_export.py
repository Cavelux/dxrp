...
ROLE_SUFFIXES = {
    ...
    "nrm": ["_nrm", "_normal", "_n"],
    "rma": ["_rma", "_orm", "_arm", "_mra", "_aorm"],
    "rough": ["_rough", "_roughness", "_r"],
    "metal": ["_metal", "_metallic", "_m"],
    "ao": ["_ao", "_occlusion"],
    "emissive": ["_emissive", "_emis", "_emm", "_glow", "_e"],
    # "_o" is single-letter but tested after "_ao"/"_occlusion", so an AO map can't claim it.
    "opacity": ["_opacity", "_opac", "_mask", "_alpha", "_o"],
...
# "aorm" is the same layout as "orm" (the A is the AO the O already names) and must come
# FIRST: the alternation is ordered, so a shorter match would claim the token instead.
RMA_ORDERS = ["aorm", "rma", "orm", "arm", "mra"]
...
def list_static_meshes(paths):
    ...
    seen = set()
    for base in paths:
        log("Listing assets under {}...".format(base))
        obj_paths = eal.list_assets(base, recursive=True, include_folder=False)
        for i, obj_path in enumerate(obj_paths):
            if obj_path in seen:
                continue
            # load_asset pulls the whole package off disk - minutes for a big folder, and
            # the s&box tool has nothing to show for it unless we say where we are.
            if i % 25 == 0:
                log("Loading asset {}/{}...".format(i + 1, len(obj_paths)))
...
def load_selection(asset_paths):
    ...
    for i, p in enumerate(asset_paths):
        # One log line per asset: loading a big selection is a long silent stretch otherwise,
        # and these are cheap next to the package load itself.
        log("Loading {}/{}: {}".format(i + 1, len(asset_paths), p.rsplit("/", 1)[-1]))
...
def collect_scene(actors):
    ...
    log("Walking {} level actors...".format(len(actors)))
    for i, actor in enumerate(actors):
        # A World Partition level is tens of thousands of actors; report often enough that
        # the s&box side can tell walking from hanging.
        if i and i % 250 == 0:
            log("Walked {}/{} actors ({} placements so far)".format(i, len(actors), len(placements)))
...
def scene_mode(map_path, out_dir, limit):
    ...
    log("Loading map {} (this is the slowest step for a big level)...".format(map_path))
...
def main():
    ...
    if assets_file and os.path.exists(assets_file):
        ...
        log("Loading {} selected asset(s)...".format(len(asset_paths)))
